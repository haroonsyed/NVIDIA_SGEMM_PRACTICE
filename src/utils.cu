...
void test_haroon_mysgemm_v1(int M, int N, int K, float alpha, float *A, float *B, float beta, float *C) {
    const int THREADS_PER_BLOCK_X = 32;
    const int THREADS_PER_BLOCK_Y = 32;
    dim3 block_dim(THREADS_PER_BLOCK_X, THREADS_PER_BLOCK_Y, 1);
    dim3 grid_dim((N / block_dim.x) + 1, (M / block_dim.y) + 1, 1);
    matrix_multiply_kernel_1<<<grid_dim, block_dim>>>(A, M, K, B, K, N, C, M, N);
}
...
void test_haroon_mysgemm_v2(int M, int N, int K, float alpha, float *A, float *B, float beta, float *C) {
    dim3 block_dim(32, 32);
    dim3 grid_dim((N / 32) + 1, (M / 32) + 1, 1);
    haroon_mysgemm_v2<<<grid_dim, block_dim>>>(A, M, K, B, K, N, C, M, N);
}
...
void test_kernel(int kernel_num, int M, int N, int K, float alpha, float *A, float *B, float beta, float *C,
    ...
    switch (kernel_num) {
        ...
        case 7:
            ...
            break;
        case 8:
            test_haroon_mysgemm_v1(M, N, K, alpha, A, B, beta, C);
            break;
        case 9:
        ...
        case 10:
            ...
            break;
        case 11:
            test_haroon_mysgemm_v4(M, N, K, alpha, A, B, beta, C);
            break;
        default:
            break;
    }
